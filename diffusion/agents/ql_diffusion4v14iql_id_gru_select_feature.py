...
from diffusion.agents.meta_diffusion_id_TD7_onlyzs import Diffusion
from diffusion.agents.model import MLP
from diffusion.agents.model import MLP_GRU, Meta_MLP_GRU, Meta_MLP_GRU_v1, Meta_MLP_GRU_select_feature, MLP_GRU_select_feature
...
class Diffusion_QL(object):
    def __init__(self,
        ...
        # self.model = Meta_MLP_GRU_select_feature(state_dim=state_dim, action_dim=action_dim, z_dim=10, device=device)
        self.model = MLP_GRU_select_feature(state_dim=state_dim, action_dim=action_dim, device=device)
    ...
    def train(self, replay_buffer, iterations, batch_size=100, log_writer=None):
        ...
        for _ in range(iterations):
            ...
            """ Policy Training """
            bc_loss = self.actor.loss(action, state, zs)
            actor_loss = bc_loss
    ...
    def sample_action(self, state, z):
        ...
        z_rpt = torch.repeat_interleave(z.to(self.device), repeats=10, dim=0)
        with torch.no_grad():
            action = self.actor.sample(state_rpt, z_rpt)
            # q_value = self.critic.q0(action / 1e6, state_rpt).flatten()
            q_value = self.critic.q0(action, state_rpt).flatten()
